...
public class ShapeImageView extends ImageView {
    ...
    private boolean mBorderOverLap;  //边框叠加
    ...
    public ShapeImageView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mBorderOverLap = a.getBoolean(R.styleable.ShapeImageView_siv_border_overlap, DEFAULT_BORDER_OVERLAY);
    ...
    private Bitmap getBitmapFormDrawable() {
        ...
        if (drawable instanceof BitmapDrawable) {//如果是BitmapDrawable就直接拿出bitmap
            return ((BitmapDrawable) drawable).getBitmap();
        }
        try {
            //ColorDrawable的getIntrinsicWidth()返回的-1 所以说给DEFAULT_DRAWABLE_DIMENSION默认值为2;
    ...
    private void updateSetup() {
        ...
        //设置BitmapShader 第二个参数传入拉伸模式
        BitmapShader bitmapShader = new BitmapShader(mBitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
        mBitmapPaint.setAntiAlias(true);
        //给BitmapPaint设置bitmapShader(给图形贴图)
        ...
        if (!mBorderOverLap && mBorderWidth > 0) {
            //允许边框重叠
    ...
    //更新矩阵 类似于CENTER_CROP的计算。。scaleType设置其他的类型可能不起作用
